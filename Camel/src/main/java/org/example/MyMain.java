    ...
    public static void main(String[] args) throws Exception {
        ...
        context.addRoutes(new RouteBuilder() {
          ...
          private Object computeSum(Exchange exchange) {
              String[] splitted = exchange.getIn().getBody().toString().split(" ");
              Integer sum = Integer.valueOf(splitted[0])+Integer.valueOf(splitted[1]);
              exchange.getIn().setBody("Result: "+sum);
              return exchange.getIn().getBody();
          }
          private boolean notError(Exchange exchange) {
            return ! exchange.getIn().getBody().toString().startsWith("Error");
          }
