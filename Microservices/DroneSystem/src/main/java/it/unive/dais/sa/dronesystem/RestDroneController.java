...
public class RestDroneController {
    ...
    @PostMapping(path = "drones/position/{id}")
    public Position updateDrone(@PathVariable Integer id, @RequestBody Position new_position) {
        Drone drone = drones.findById(id).get();
        drone.setPosition(new_position);
        Drone result = drones.save(drone);
        return new_position;
    }

    @GetMapping("/drones/position/{id}")
    public Position getDronePosition(@PathVariable Integer id) {
        return drones.findById(id).get().getPosition();
    }
